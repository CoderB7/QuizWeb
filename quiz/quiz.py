...
def quiz_list():
    ...
    print(user.quizs)
    return render_template(
        "quiz/quiz_list.html", user_quizs=user.quizs, number_of_quizs=(len(user.quizs))
    )
...
def solve_quiz(q_id):
    ...
    if current_user.is_authenticated:
        if request.method == "POST":
            ...
            return render_template(
                "quiz/quiz_solve_from_zero.html",
                current_user=current_user,
                quiz_id=q_id,
                quiz_data=question_data,
